...
#This program automatically copies CSV files from a landing bucket, 
#masks sensitive data using Google Cloud DLP, and saves the protected files in a raw bucket.

# Function to build a DLP Table from CSV content
...
# Function to convert a DLP Table back to CSV format
...
# The main function that orchestrates the job of reading CSV files from the landing bucket, 
# processing them with DLP, and writing the transformed files to the raw bucket.
def run_job() -> None:
    ...
    #call the Google Cloud DLP service.
    ...
    # Find all CSV files in the landing bucket and process them
    ...
    for blob in blobs:
        ...
        # Build a DLP Table from the CSV content
        table = build_table_from_csv(csv_text)

        # Call the DLP API.The DLP service reads the template and follows its rules.
        ...
        # Get the transformed table from the response and convert it back to CSV
        ...
        # Save the transformed CSV to the raw bucket
